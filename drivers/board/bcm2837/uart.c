...
void uart_init(void)
{
  ...
  mmio_write32(AUX_ENABLES, 1);                   //Enable mini uart (this also enables access to its registers)
  mmio_write32(AUX_MU_CNTL_REG, 0);               //Disable auto flow control and disable receiver and transmitter (for now)
  mmio_write32(AUX_MU_IER_REG, 0);                //Disable receive and transmit interrupts
  ...
  mmio_write32(AUX_MU_CNTL_REG, 3);               //Finally, enable transmitter and receiver
}
