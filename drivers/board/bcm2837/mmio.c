...
void mmio_write8(const uint32_t reg, const uint8_t data) { *(volatile uint8_t *)((uintptr_t)MMIO_BASE + reg) = data; }
...
uint16_t mmio_read16(const uint32_t reg) { return *(volatile uint16_t *)((uintptr_t)MMIO_BASE + reg); }

uint32_t mmio_read32(const uint32_t reg) { return *(volatile uint32_t *)((uintptr_t)MMIO_BASE + reg); }

uint64_t mmio_read64(const uint32_t reg) { return *(volatile uint64_t *)((uintptr_t)MMIO_BASE + reg); }
